...
void init_speaker(gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, uint32_t sample_rate)
{
    ...
    i2s_std_config_t i2s_tx_cfg = {
        ...
        .slot_cfg = I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(
            I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO), // 标准模式(飞利浦模式),采样率位深,单通道/双通道
        .gpio_cfg =                                        // 配置引脚参数
        {
            .mclk = I2S_GPIO_UNUSED, // 忽略, 用外部的设备当主时钟才需要
            .bclk = bclk,            // BCK引脚
            .ws = ws,                // WS引脚 声道选择线
            .dout = dout,            // DOUT引脚 喇叭播放
            .invert_flags =          // 引脚翻转
            {
                .mclk_inv = false, // 忽略
...
void init_pdm_microphone(gpio_num_t dat, gpio_num_t clk, uint32_t sample_rate)
{
    ...
    i2s_pdm_rx_config_t pdm_rx_cfg = {
        ...
        .slot_cfg = I2S_PDM_RX_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO), // 通道,位深 通道
        .gpio_cfg =
            {
                .clk = clk, // 时钟信号
                .din = dat, // 数据信号 录音
    ...
    pdm_rx_cfg.slot_cfg.slot_mask = I2S_STD_SLOT_RIGHT; // 手动赋值 右声道
...
{
    size_t bytes_read;
    i2s_channel_read(rx_handle, (char *)data, samples * sizeof(int16_t), &bytes_read, 1000);
...
int audio_write(const int16_t *data, int samples) 
{
	...
	return i2s_channel_write(tx_handle, (char *)data, samples * sizeof(int16_t), &bytes_written, 1000);
